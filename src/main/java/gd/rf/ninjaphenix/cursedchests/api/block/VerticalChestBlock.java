...
public abstract class VerticalChestBlock extends BlockWithEntity implements Waterloggable, InventoryProvider
{
    ...
    {
        @Override
        public SidedInventory getFromDoubleChest(VerticalChestBlockEntity bottomChestBlockEntity, VerticalChestBlockEntity topChestBlockEntity)
        { return new DoubleSidedInventory(bottomChestBlockEntity, topChestBlockEntity); }

        @Override
        public SidedInventory getFromSingleChest(VerticalChestBlockEntity chestBlockEntity) { return chestBlockEntity; }
    };
    ...
    {
        @Override
        public Component getFromDoubleChest(VerticalChestBlockEntity bottomChestBlockEntity, VerticalChestBlockEntity topChestBlockEntity)
        {
        ...
        @Override
        public Component getFromSingleChest(VerticalChestBlockEntity chestBlockEntity) { return chestBlockEntity.getDisplayName(); }
    };
    ...
    private static <T> T retrieve(BlockState state_1, IWorld world, BlockPos pos_1, PropertyRetriever<T> var_1)
    {
        ...
        if (state_2.getBlock() == state_1.getBlock())
        {
            ...
            if (chestType_2 != VerticalChestType.SINGLE && chestType_1 != chestType_2 && state_2.get(FACING) == state_1.get(FACING))
            {
                ...
                {
                    VerticalChestBlockEntity chestBlockEntity_2 =
                            chestType_1 == VerticalChestType.TOP ? chestBlockEntity1 : (VerticalChestBlockEntity) blockEntity_2;
                    VerticalChestBlockEntity chestBlockEntity_3 =
                            chestType_1 == VerticalChestType.TOP ? (VerticalChestBlockEntity) blockEntity_2 : chestBlockEntity1;
    ...
    public BlockState getPlacementState(ItemPlacementContext context)
    {
        ...
        if (direction_2.getAxis().isVertical() && sneaking)
        {
            ...
            if (direction_3 != null && direction_3.getAxis() != direction_2.getAxis() && direction_3 == direction_1)
                chestType = direction_2 == Direction.UP ? VerticalChestType.TOP : VerticalChestType.BOTTOM;
        }
        else if (!sneaking)
        {
            BlockState aboveBlockState = world.getBlockState(pos.offset(Direction.UP));
            if (aboveBlockState.getBlock() == this && aboveBlockState.get(TYPE) == VerticalChestType.SINGLE && aboveBlockState.get(FACING) == direction_1)
                chestType = VerticalChestType.BOTTOM;
            else
            {
                BlockState belowBlockState = world.getBlockState(pos.offset(Direction.DOWN));
                if (belowBlockState.getBlock() == this && belowBlockState.get(TYPE) == VerticalChestType.SINGLE && belowBlockState.get(FACING) == direction_1)
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState otherState, IWorld world, BlockPos pos, BlockPos otherPos)
    {
        ...
        if (state.get(TYPE) == VerticalChestType.TOP && world.getBlockState(pos.offset(Direction.DOWN)).getBlock() != this)
            return state.with(TYPE, VerticalChestType.SINGLE);
        if (state.get(TYPE) == VerticalChestType.BOTTOM && world.getBlockState(pos.offset(Direction.UP)).getBlock() != this)
            return state.with(TYPE, VerticalChestType.SINGLE);
