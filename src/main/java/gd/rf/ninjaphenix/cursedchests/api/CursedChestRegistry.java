...
public class CursedChestRegistry
{
    ...
    public static void registerChest(VerticalChestBlock block)
    {
        ...
        {
            if (block2blockEntityMap.containsKey(block))
            {
                System.out.println("Chest type " + block.getName() + " has already been registered!");
                return;
            }
    ...
    /**
     * Gets an instance of a registered chest block's block entity. Used in {@link gd.rf.ninjaphenix.cursedchests.mixins.ItemDynamicRenderer} to render the
    ...
    public static BlockEntity getChestBlockEntity(VerticalChestBlock block) { return block2blockEntityMap.get(block); }
}
